...
async def give_filter(client, message):
    ai_search = True
    reply_msg = await message.reply_text(f"<b><i>Sᴇᴀʀᴄʜɪɴɢ Fᴏʀ {message.text} ♻️</i></b>")
...
async def pm_text(bot, message):
    ...
    ai_search = True
    reply_msg = await bot.send_message(message.from_user.id, f"<b><i>Sᴇᴀʀᴄʜɪɴɢ Fᴏʀ {content} ♻️</i></b>", reply_to_message_id=message.id)
...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        ai_search = True
        reply_msg = await query.message.edit_text(f"<b><i>Sᴇᴀʀᴄʜɪɴɢ Fᴏʀ {movie} ♻️</i></b>")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        text = "<b>👨‍💻 Hᴏᴡ Tᴏ Usᴇ Oᴜʀ Bᴏᴛ :-\n\n🔻 /start - Cʜᴇᴄᴋ Bᴏᴛ Is Wᴏʀᴋɪɴɢ Oʀ Nᴏᴛ.\n\n🔻 /stats - Cʜᴇᴄᴋ Bᴏᴛ Fɪʟᴇs Aɴs Usᴇʀs.\n\n🔻 /settings - Cᴏɴғɪɢᴜʀᴇ Cʟᴏɴᴇ Bᴏᴛ Sᴇᴛᴛɪɴɢ ( Oᴡɴᴇʀ Oɴʟʏ ).\n\n🔻 /reset - Rᴇsᴇʀ Aʟʟ Sᴇᴛᴛɪɴɢs Tᴏ Dᴇғᴀᴜʟᴛ Oʀ Nᴏɴᴇ ( Oᴡɴᴇʀ Oɴʟʏ ).\n\n🔻 /broadcast - Bʀᴏᴀᴅᴄᴀsᴛ A Mᴇssᴀɢᴇ Tᴏ Yᴏᴜʀ Bᴏᴛ Usᴇʀs ( Oᴡɴᴇʀ Oɴʟʏ ).</b>"
        btn = [[
        ...
        if settings["update_channel_link"] != None:
            buttons.append([[InlineKeyboardButton('⛔Jᴏɪɴ Mᴀɪɴ Cʜᴀɴɴᴇʟ⛔', url=f'{settings["update_channel_link"]}')]])
    ...
    if query.data.startswith("file"):
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
        await query.answer(f"Hᴇʏ {query.from_user.first_name}, Aʟʟ Fɪʟᴇs Oɴ Tʜɪs Pᴀɢᴇ Hᴀs Bᴇᴇɴ Sᴇɴᴛ Sᴜᴄᴄᴇғᴜʟʟʏ Tᴏ Yᴏᴜʀ Pᴍ !", show_alert=True)
       ...
       # if not search:
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
        await query.answer(f"Hᴇʏ {query.from_user.first_name}, Aʟʟ Fɪʟᴇs Oɴ Tʜɪs Pᴀɢᴇ Hᴀs Bᴇᴇɴ Sᴇɴᴛ Sᴜᴄᴄᴇsғᴜʟʟʏ Tᴏ Yᴏᴜʀ Pᴍ !", show_alert=True)
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>Aᴅᴠᴀɴᴄᴇ Aɪ Oғ CᴘFʟɪᴄᴋs Tʀʏ Tᴏ Fɪɴᴅ Yᴏᴜʀ Mᴏᴠɪʀ Wɪᴛʜ Yᴏᴜʀ Wʀᴏɴɢ Sᴘᴇʟʟɪɴɢ.</i></b>")
        movienamelist = []
